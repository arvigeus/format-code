...
type Props = HTMLProps<"div"> & {
  ...
  readOnly?: boolean;
  required?: boolean;
  onClick?: (e: MouseEvent<HTMLTextAreaElement>) => unknown;
...
export default class Editor extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    if (/Win/i.test(navigator.userAgentData.platform))
      this._platform = PLATFORM_WINDOWS;
    else if (/(Mac|iPhone|iPod|iPad)/i.test(navigator.userAgentData.platform))
      this._platform = PLATFORM_MAC;
  }
  ...
  _recordChange = (record: Record, overwrite: boolean | null = false) => {
    ...
    if (overwrite) {
      ...
      if (last && timestamp - last.timestamp < HISTORY_TIME_GAP) {
        ...
        const previousLine = this._getLines(
          last.value,
          last.selectionStart
        ).pop();

        const previous = previousLine?.match(re);

        // Get the current line
        const currentLine = this._getLines(
          record.value,
          record.selectionStart
        ).pop();

        const current = currentLine?.match(re);
  ...
  _handleKeyDown = (e: any) => {
    ...
    if (e.keyCode === KEYCODE_TAB && !ignoreTabKey && this.state.capture) {
      ...
      (this._platform === PLATFORM_MAC // Trigger undo with ⌘+Z on Mac
      ...
      (this._platform === PLATFORM_MAC // Trigger redo with ⌘+Shift+Z on Mac
        ? e.metaKey && e.keyCode === KEYCODE_Z && e.shiftKey
        : this._platform === PLATFORM_WINDOWS // Trigger redo with Ctrl+Y on Windows
      ...
      e.keyCode === KEYCODE_M &&
      e.ctrlKey &&
      (this._platform === PLATFORM_MAC ? e.shiftKey : true)
    ) {
      e.preventDefault();
